...
**What it sends.** One request per play, and during a bulk capture one per best score it sends (the song, chart type and level, the score, when it was read, and that it came from the song list — no judgments), to PIU Scores, containing exactly: which mix, the song title, chart type and level, the five judgment counts, max combo, the score, whether the run was broken, when it was played, and which mode captured it. No screenshot is ever uploaded. Nothing is sent anywhere else, and nothing is sent about the machine, the player or the session.
...
**No telemetry.** The watcher does not phone home. Its only outbound traffic is the plays it posts to PIU Scores, the chart list and the player's own bests it reads from PIU Scores to match titles and skip bests already there, and its check of GitHub for a newer version at start-up.
