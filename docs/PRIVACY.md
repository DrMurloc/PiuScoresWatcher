...
**What it keeps.** Under `%LOCALAPPDATA%\PiuScoresWatcher\`: the settings (mode, Start with Windows, the screenshots folder), seven days of logs, and the result screens it could not read — those stay on the machine until the player chooses to send one to the developer with the button. The PIU Scores token is stored encrypted for the Windows account it was entered under; it is never written in clear and never leaves the machine except as the credential on the requests above.
