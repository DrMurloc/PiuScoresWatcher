...
- **Release**: push a tag `vX.Y.Z`; `.github/workflows/release.yml` builds, tests, packages, signs when the Artifact Signing variables exist, and publishes the GitHub release. Never hand-upload a release.
...
- **Feedback is a notification and a sound** — never a window that takes focus from the game, and **never an overlay** of any kind (design D3). Each play plays bulk capture's chime, low tone or tick (D63, D64), and its notification goes silent when it does. **Every kind of notification can be switched off**, all at once or one by one (D35), and the sounds with one switch; state that matters (not connected, paused) is always visible in the tray menu's status line and the settings window, so switching notifications off never hides a problem.
